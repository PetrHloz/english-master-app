...
def analyze_text(text):
    ...
    Colloquial (General): [Common slang/informal alternatives in US/UK]
    Common Mistake: [Typical learner errors for this specific phrase]
    Scottish English (Scots/Informal): [How a Scot would say it, include phrases like 'whit happens', 'hen/lad', etc.]
    Cultural Context: [Brief cultural/historical insight, specifically regarding Scotland if applicable]
...
if submit_button and user_input:
    with st.spinner('Analyzuji komplexní rozbor...'):
        try:
            res = analyze_text(user_input)
            
            def safe_get(key):
                data = res.get(key, "")
                if isinstance(data, (list, dict)): return str(data)
            ...
            f_details = details.replace('Meaning:', '<span class="section-header">Meaning:</span>')\
            ...
            st.markdown(f"<div class='english-box'>{f_details.replace('\n', '<br>')}</div>", unsafe_allow_html=True)

            # 5. DIALEKTY (Nové formátování)
            st.subheader("Stylistic & Dialect Corner")
            stylistic = safe_get('stylistic').replace('*', '')
            f_stylistic = stylistic.replace('Colloquial (General):', '<span class="section-header">Colloquial (General):</span>')\
                                   .replace('Common Mistake:', '<span class="section-header">Common Mistake:</span>')\
                                   .replace('Scottish English (Scots/Informal):', '<span class="section-header">Scottish English (Scots/Informal):</span>')\
                                   .replace('Cultural Context:', '<span class="section-header">Cultural Context:</span>')
            st.markdown(f"<div class='dialect-box'>{f_stylistic.replace('\n', '<br>')}</div>", unsafe_allow_html=True)
